...
do
fdirz[i]=$fdir${fnr[$i]}'/'
ffile0[i]=${fdirz[$i]}${fnr[$i]}'.nii.gz'
ffile_ph0[i]=${fdirz[$i]}${fnr[$i]}'_ph.nii.gz'
ffile[i]=${fdirz[$i]}${fnr[$i]}'-deo.nii.gz'
ffile_ph[i]=${fdirz[$i]}${fnr[$i]}'_ph-deo.nii.gz'
done
...
if ( [ ! -f ${ffile_ph[$i]} ] ); then
cp ${ffile_ph0[$i]} ${ffile_ph[$i]}
3drefit -deoblique ${ffile_ph[$i]}
fi
done
...
for (( i=0; i<nses; i++ ));
do
ffile6_ph[i]=${fdirz[$i]}${fnr[$i]}'_ph-prep.nii.gz'
if (( i == 0 )); then
3dNwarpApply -nwarp "${BWARP[$i]} ${AWARP[$i]}" \
-source ${ffile_ph[$i]} \
-master ${ffile5[0]} \
